...
class ToolsUi(QDialog):
    signal_wirte_msg = QtCore.pyqtSignal(str)
    ...
    def ui_translate(self):
        ...
        #设置各个控件显示的文字
        #也可以用控件的setText()方法设置文字
        self.setWindowTitle(self._translate("TCP-UDP","TCP/UDP网络测速工具-窗口%s" % self.num))
        self.comboBox_tcp.setItemText(0, self._translate("TCP-UDP","TCP服务端"))
        self.comboBox_tcp.setItemText(1, self._translate("TCP-UDP", "TCP服务端"))
        self.comboBox_tcp.setItemText(2, self._translate("TCP-UDP", "UDP服务端"))
        self.comboBox_tcp.setItemText(3, self._translate("TCP-UDP", "UDP服务端"))
    ...
    def connect(self):
        ...
        self.signal_wirte_msg.connect(self.write_msg)
    ...
    def comboBox_change(self):
        ...
        if self.comboBox_tcp.currentIndex() == 1 or self.comboBox_tcp.currentIndex() == 3:
            ...
            self.lineEdit_ip_send.show()
            self.textEdit_send.show()
            self.label_port.setText(self._translate('TCP-UDP', '目标端口号:'))
